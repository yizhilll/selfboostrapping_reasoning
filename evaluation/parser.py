...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    if data_name in ["math", "minerva_math"]:
        ...
        gt_cot, gt_ans = None, example["answer"]
    elif data_name in ['tal-scq5k-en']:
        gt_cot = example['answer_analysis'][0] if isinstance(example['answer_analysis'], list) else None
        gt_ans = example['answer_value']
    elif data_name == 'numina':
        gt_cot = example["solution"]
        gt_ans = extract_answer(gt_cot, data_name)
    else:
...
def parse_question(example, data_name):
    ...
    if data_name == "asdiv":
        ...
        assert example['qtype'] == 'single_choice'
        options = []
        for option_item in  example['answer_option_list']:
            option_item = option_item[0]
            options.append(f"({option_item['aoVal'].upper()}) {option_item['content'].strip()}")
        options = " \n".join(options)
...
def _test_numina_answer():
    from datasets import load_dataset
    dataset = load_dataset('AI-MO/NuminaMath-CoT')
    for split in ['train', 'test']:
        print('='*10, f'extract {split} answer', '='*10)
        for text in dataset[split][:10]['solution']:    
            print('original text:')
            print(text)
            print('extracted answer:')
            print(extract_answer(text, "numina", use_last_number=False))
            print('-'*30)
...
def _test_extract2():
    item = {
        "uid": "14dfd8a5-2a89-4c5d-9e52-8b704898d8eb",
        "query": "Given $a+b=3$ and $ab=2$, find the value of the algebraic expression $a^{2}b+2a^{2}b^{2}+ab^{3}$.",
        "response": "18",
        "thought": "Given $a+b=3$ and $ab=2$, we are asked to find the value of the algebraic expression $a^{2}b+2a^{2}b^{2}+ab^{3}$.\n\nLet's break this down step by step:\n\n1. We start with the given expression: $a^{2}b+2a^{2}b^{2}+ab^{3}$.\n\n2. Notice that we can factor out $ab$ from each term: $ab(a^{2}+2ab+b^{2})$.\n\n3. Recognizing that $a^{2}+2ab+b^{2}$ is a perfect square, we rewrite it as $(a+b)^{2}$: $ab(a+b)^{2}$.\n\n4. Substituting the given values for $ab$ and $a+b$, we get: $2 \\times 3^{2}$.\n\n5. Performing the multiplication and squaring, we find: $2 \\times 9 = 18$.\n\nTherefore, the value of the algebraic expression $a^{2}b+2a^{2}b^{2}+ab^{3}$ is $\\boxed{18}$.",
        "query_len": 97,
        "response_len": 97,
        "thought_len": 653,
        "quality": 9,
        ...
        "answer_type": "a",
        "cases": "",
        "infer_bon64": "<Thought>\nAlright, let's tackle this problem step by step. We are given two equations:\n\n1. \\( a + b = 3 \\)\n2. \\( ab = 2 \\)\n\nOur goal is to find the value of the expression \\( a^{2}b + 2a^{2}b^{2} + ab^{3} \\).\n\nFirst, let's understand what we're dealing with. We have a system of linear equations with two variables, \\( a \\) and \\( b \\). Our first thought is to solve for one variable in terms of the other using these equations.\n\nFrom equation (1), we can express \\( a \\) as:\n\\[ a = 3 - b \\]\n\nNow, substitute this expression for \\( a \\) into equation (2):\n\\[ (3 - b)b = 2 \\]\n\\[ 3b - b^{2} = 2 \\]\n\\[ -b^{2} + 3b - 2 = 0 \\]\nLet's rearrange the equation:\n\\[ b^{2} - 3b + 2 = 0 \\]\n\nNow, we need to solve this quadratic equation for \\( b \\). Let's factor it:\n\\[ (b - 1)(b - 2) = 0 \\]\nSo, the solutions are:\n\\[ b = 1 \\] or \\( b = 2 \\)\n\nNow, let's find the corresponding values of \\( a \\) for each \\( b \\):\n\n1. If \\( b = 1 \\):\n\\[ a = 3 - 1 = 2 \\]\nSo, \\( a = 2 \\) and \\( b = 1 \\)\n\n2. If \\( b = 2 \\):\n\\[ a = 3 - 2 = 1 \\]\nSo, \\( a = 1 \\) and \\( b = 2 \\)\n\nNow that we have the pairs of \\( (a, b) \\), let's compute the value of the expression \\( a^{2}b + 2a^{2}b^{2} + ab^{3} \\) for both cases.\n\n**Case 1:** \\( a = 2 \\), \\( b = 1 \\)\n\\[ a^{2}b + 2a^{2}b^{2} + ab^{3} = 2^{2}(1) + 2(2)^{2}(1)^{2} + 2(1)(1)^{3} \\]\n\\[ = 4 + 2(4)(1) + 2(1)(1) \\]\n\\[ = 4 + 8 + 2 \\]\n\\[ = 14 \\]\n\n**Case 2:** \\( a = 1 \\), \\( b = 2 \\)\n\\[ a^{2}b + 2a^{2}b^{2} + ab^{3} = 1^{2}(2) + 2(1)^{2}(2)^{2} + 1(2)(2)^{3} \\]\n\\[ = 2 + 2(1)(4) + 2(2)(8) \\]\n\\[ = 2 + 8 + 32 \\]\n\\[ = 42 \\]\n\nInterestingly, both cases yield the same result, which means the expression evaluates to **14** regardless of whether \\( a = 2 \\) and \\( b = 1 \\) or \\( a = 1 \\) and \\( b = 2 \\).\n</Thought>\n\n<Output>\nThe value of the expression \\( a^{2}b + 2a^{2}b^{2} + ab^{3} \\) is **14**.\n</Output>"
    }
    text=item["infer_bon64"]
    print(extract_answer(item['response'],data_name='N/A', use_last_number=True))
    print(extract_answer(text,data_name='N/A', use_last_number=True))
