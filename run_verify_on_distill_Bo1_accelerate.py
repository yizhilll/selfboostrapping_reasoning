...
with open("OpenO1-SFT-Ultra/uid_to_meta.pkl", "rb") as f:
    uid_to_meta = pickle.load(f)
...
def process_file(file_path, compute_correctness=False, batch_size=10, show_progress=True, max_workers=10):
    ...
    # prepend info in uid_to_meta to each item, if not exists, set to 'Math' and 'a'
    # keys: 'query_len': 123, 'response_len': 7, 'thought_len': 0, 'quality': 9, 'difficulty': 8, 'source': 'WebInstructFull_pak', 'Topic': 'Math', 'answer_type': 'b', 'cases': ''
    keys = ['query_len', 'response_len', 'thought_len', 'quality', 'difficulty', 'source', 'Topic', 'answer_type', 'cases']
    for item in data:
        if item['uid'] not in uid_to_meta:  
            meta = {'Topic': 'Math', 'answer_type': 'a', 'source': 'N/A'}
        else:
            meta = uid_to_meta[item['uid']]
        for key in keys:
            item[key] = meta[key]
